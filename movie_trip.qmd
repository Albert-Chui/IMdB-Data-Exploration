...
ggplot(series_timed_runlength)+
  ...
  ylim(0, 60)
```
Visually, it seems that has not chnaged over time. 
...
```{r, echo = F}
runtime_df <- movies %>% filter(runtimeMinutes > 0 & titleType == "movie") %>% select(startYear, runtimeMinutes) 
...
$$
```{r, echo = F}
runtime_df <- movies %>% filter(runtimeMinutes > 0 & titleType == "tvSeries") %>% select(startYear, runtimeMinutes) 
