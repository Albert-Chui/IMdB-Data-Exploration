...
aov(averageRating ~ startYear, data = aov_df) %>% summary()
...
```
# Testing for differences between Runtime and Time for Movies and TV series
```{r, echo = F, warning = F}
...
ggplot(timed_runlength)+
  ...
  ylim(0, 120)
```
Visually, it seems that movies lengths have not increased or decreased over time. 
...
ggplot(series_timed_runlength)+
  ...
  ylim(0, 60)
```
Visually, it seems that has not chnaged over time on average. 

## Conduct ANOVA Test for average runtime of Movies Over Time

## Check Assumptions
boxplot for 131 years. 
...
```
## Conduct ANOVA Test for average runtime of TV Series Over Time
...
runtime_df <- movies %>% filter(runtimeMinutes > 0 & titleType == "tvSeries") %>% select(startYear, runtimeMinutes) %>% na.omit()

aov(runtimeMinutes ~ startYear, data = runtime_df) %>% anova()
...
# Testing for differences in Adult Movies vs Rating
